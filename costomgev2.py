...
# Function to validate the SA ID number using the Luhn checksum
def validate_sa_id(id_number):
    """Validate a South African ID number based on Luhn checksum."""
    # Extract the first 12 digits (excluding the checksum digit)
    id_without_checksum = id_number[:-1]
    
    # Calculate the Luhn checksum for the first 12 digits
    calculated_checksum = calculate_luhn_left_to_right(id_without_checksum)
    
    # Compare with the actual checksum (last digit)
    return str(calculated_checksum) == id_number[-1]

# Function to generate all valid ID numbers and save them to a text file
def generate_all_valid_ids(start_year=1900, end_year=2023, file_name="valid_ids_with_library.txt"):
                    ...
                    for day in range(1, days_in_month + 1):  # DD (01-31)
                        ...
                        for gender in range(0, 10000):  # SSSS (0000-9999)
                            ...
                            for citizenship in range(0, 2):  # C (0 or 1)
                                ...
                                # First validate the ID using the Luhn algorithm
                                if validate_sa_id(valid_id):
                                    # Validate the ID using the external library
                                    try:
